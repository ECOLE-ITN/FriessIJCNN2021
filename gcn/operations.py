...
class GraphConv(keras.layers.Layer):
    ...
    def __init__(self, adjacency_matrix, input_features=1, filters=1):
    ...
    def build(self, input_shape):
        ...
        self.w = tf.Variable(initial_value=w_init(shape=(self.input_features, self.filters), dtype="float32"),
                             trainable=True, )

        # Calculate the normalized adjacency matrix
        A_tilde = self.A + np.identity(self.A.shape[0])
        D = np.diag(np.sum(A_tilde, axis=0))
        D_sqrt = fractional_matrix_power(D, -0.5)

        self.A_hat = tf.constant(D_sqrt.dot(A_tilde).dot(D_sqrt), dtype='float32')
    ...
    def call(self, inputs):
        ...
        X = inputs
        #W = tf.random_normal((self.input_features, self.filters))
        XW = tf.reshape(tf.reshape(X, [-1, self.input_features]) @ self.w, [-1, self.A.shape[0], self.filters])


        A_expand = tf.tile(self.A_hat[None], [batch_size, 1, 1])
        return tf.matmul(A_expand, XW)
        #return tf.matmul(A_expand, tf.matmul(inputs, self.w))
